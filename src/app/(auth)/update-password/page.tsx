...
function UpdatePasswordContent() {
  ...
  useEffect(() => {
    // If there is a code, exchange it for a session so the user can update their password
    if (code) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use Supabase native updateUser to change password since we exchanged the code
      ...
      if (updateError) {
        ...
        setSuccess(true);
        // Automatically sign out after password change for security
  ...
  if (success) {
    return (
      <div className="w-full max-w-md p-8 bg-white rounded-2xl shadow-sm border border-[#E85D9A]/20 text-center">
        ...
        <h1 className="text-2xl font-bold text-[#4A1B3C] mb-2">Password Updated Successfully</h1>
